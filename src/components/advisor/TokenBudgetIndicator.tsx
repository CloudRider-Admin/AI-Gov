...
export function TokenBudgetIndicator({ refreshKey }: TokenBudgetIndicatorProps) {
  ...
  const barColor = pct > 80 ? 'bg-terminal-red' : pct > 50 ? 'bg-terminal-amber' : 'bg-terminal-green';
  const textColor = pct > 80 ? 'text-terminal-red' : pct > 50 ? 'text-terminal-amber' : 'text-terminal-muted';
