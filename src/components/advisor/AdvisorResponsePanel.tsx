...
import { Clock, Copy, Check, Lock } from 'lucide-react';
    ...
    <div className="border-t border-terminal-border pt-6 animate-slide-up space-y-6">
      ...
      {response.mode === 'clarification' ? (
        <>
          ...
          <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-5 dark:border-blue-800 dark:bg-blue-950/20">
            <div className="flex items-start gap-3">
              <span className="text-2xl">💬</span>
              <div className="flex-1">
                <p className="text-sm font-semibold text-blue-900 dark:text-blue-200 font-mono">
                  I need a bit more context to give you a tailored assessment
                </p>
                <p className="mt-2 text-sm text-blue-700 dark:text-blue-300 font-mono leading-relaxed">
                ...
                {response.followUpQuestions.map((question, idx) => (
                  ...
                  >
                    ...
                    <textarea
                      ...
                      className="w-full bg-terminal-bg/50 border border-terminal-border rounded px-3 py-2 text-sm font-mono text-terminal-text placeholder:text-terminal-muted/40 resize-none min-h-[60px] focus:outline-none focus:border-terminal-green transition-colors"
                      rows={2}
                    />
                  </div>
                ))}
        ...
        <>
          ...
          />
          {response.gated ? (
            <div className="mb-6 border border-dashed border-terminal-border rounded-md p-4 bg-terminal-gray/10 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <Lock className="w-4 h-4 text-terminal-muted shrink-0" />
                <div>
                  <p className="text-sm font-mono text-terminal-muted">Regulatory mapping hidden</p>
                  <p className="text-xs text-terminal-muted/60 font-mono mt-0.5">GDPR, EU AI Act & NIST analysis available on Pro</p>
                </div>
              </div>
              <Link
                href="/pricing"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {response.orchestratorError.retryable && onActionCard && (
                    <button
                      ...
                      className="mt-2 inline-flex items-center gap-1 rounded-md bg-amber-100 px-3 py-1.5 text-xs font-medium text-amber-800 hover:bg-amber-200 dark:bg-amber-900/50 dark:text-amber-200 dark:hover:bg-amber-900/70 transition-colors"
                    >
                      Retry generation
                    </button>
                  )}
