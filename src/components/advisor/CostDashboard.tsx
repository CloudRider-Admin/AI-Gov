            ...
            {cost.dailyBreakdown.map((d) => {
              ...
              return (
                <div key={d.day} className="flex-1 flex flex-col items-center gap-1" title={`${dayLabel}: ${formatTokens(total)} tokens, ${d.queries} queries`}>
                  ...
                  <span className="text-[10px] font-mono text-terminal-muted">{dayLabel}</span>
                </div>
              );
            })}
          </div>
          <div className="flex items-center gap-4 mt-2 text-[10px] font-mono text-terminal-muted">
            <span className="flex items-center gap-1"><span className="w-2 h-2 bg-terminal-green rounded-sm" /> Input</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 bg-terminal-green/60 rounded-sm" /> Output</span>
          </div>
        </div>
      )}
    </div>
