...
export function IntentFeedback({
  ...
  if (state === 'confirmed') {
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-terminal-green/10 border border-terminal-green/20 text-xs font-mono text-terminal-green">
  ...
  if (state === 'corrected' && selectedIntent) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs font-mono text-blue-400">
        <Check className="w-3.5 h-3.5" />
  ...
  if (state === 'correcting') {
    return (
      <div className="px-3 py-2 rounded-lg bg-terminal-gray/30 border border-terminal-border text-xs font-mono">
        ...
        <div className="flex flex-wrap gap-2">
          {(Object.entries(INTENT_LABELS) as [IntentType, string][])
            ...
            .map(([key, label]) => (
              <button
                ...
                className="px-3 py-1.5 rounded border border-terminal-border text-terminal-text hover:border-terminal-green hover:text-terminal-green transition-colors"
              >
                {label}
              </button>
            ))}
          <button
            ...
            className="px-3 py-1.5 rounded text-terminal-muted hover:text-terminal-text transition-colors"
          >
            Cancel
          </button>
        </div>
  ...
  // pending state
  return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-terminal-gray/20 border border-terminal-border text-xs font-mono">
      ...
      <div className="flex items-center gap-1 ml-auto">
        <button
          ...
          className="p-1 rounded hover:bg-terminal-green/20 text-terminal-green transition-colors"
          title="Correct"
        >
          <Check className="w-3.5 h-3.5" />
        </button>
        <button
          ...
          className="p-1 rounded hover:bg-red-500/20 text-terminal-muted hover:text-red-400 transition-colors"
