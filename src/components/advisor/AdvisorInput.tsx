  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6">
      ...
      {hasFollowUpPrompt && !query.trim() && (
        <div className="mb-3 px-3 py-2 rounded-md border border-blue-500/30 bg-blue-500/5">
          <p className="text-xs font-mono text-blue-400">
            <span className="text-blue-300 font-semibold">Govi asked:</span> {placeholder}
          </p>
      ...
      <div className="flex items-start gap-2 mb-4">
        ...
        <textarea
          ...
          className={`flex-grow bg-transparent border-none outline-none text-terminal-text font-mono text-sm resize-none min-h-[80px] placeholder:text-terminal-muted/50 ${
            hasFollowUpPrompt && !query.trim() ? 'ring-1 ring-blue-500/30 rounded px-2 py-1' : ''
          }`}
          disabled={isLoading}
        />
      </div>
      ...
      {validationError && (
        <p className="text-xs text-red-400 font-mono mb-2 ml-4">{validationError}</p>
      )}

      <div className="flex items-center justify-between">
        <span className={`text-xs font-mono ${isOverLimit ? 'text-red-400' : isNearLimit ? 'text-yellow-400' : 'text-terminal-muted'}`}>
          {query.length > 0
            ? isNearLimit
