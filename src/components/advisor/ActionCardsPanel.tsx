      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
        {cards.map((card) => (
          ...
          >
            ...
            <p className="text-xs font-mono text-terminal-muted">{card.description}</p>
          </button>
        ))}
      </div>
      {!isPaidUser && (
        <div className="flex items-center gap-2 text-xs font-mono text-terminal-muted">
          ...
          <span>Document generation requires a Pro subscription.</span>
          <Link href="/pricing" className="text-terminal-green hover:underline">
            Upgrade →
          </Link>
        </div>
      )}
