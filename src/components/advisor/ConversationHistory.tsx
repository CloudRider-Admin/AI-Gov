      ...
      {isOpen && (
        <div className="terminal-window h-full flex flex-col gap-2 mr-4">
          ...
          <div className="p-3 flex flex-col gap-2 overflow-y-auto flex-1">
            ...
            <button
              onClick={onNew}
              className="w-full flex items-center gap-2 px-3 py-2 mt-3 rounded border border-terminal-green/40 text-terminal-green font-mono text-xs hover:bg-terminal-green/10 transition-colors"
            >
            ...
            {conversations.map((conv) => (
              <div
                key={conv.id}
                className={`group relative rounded border cursor-pointer transition-colors ${
                  activeId === conv.id
              ...
              >
                {pendingDeleteId === conv.id ? (
                  ...
                  <div className="p-2.5 flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
                    <p className="text-xs font-mono text-red-400 leading-tight">
                      Delete this conversation?
                    </p>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => { onDelete(conv.id); setPendingDeleteId(null); }}
                        className="flex-1 text-xs font-mono px-2 py-1.5 bg-red-500/20 text-red-400 rounded border border-red-500/30 hover:bg-red-500/30 transition-colors"
                      >
                        Delete
                      </button>
                      <button
                        onClick={() => setPendingDeleteId(null)}
                        className="flex-1 text-xs font-mono px-2 py-1.5 text-terminal-muted rounded border border-terminal-border hover:text-terminal-text hover:border-terminal-green/50 transition-colors"
                      >
                        Cancel
                      </button>
                    </div>
                  ...
                  <div className="p-2" onClick={() => onSelect(conv.id)}>
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 p-1 rounded text-terminal-muted hover:text-red-400 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all"
