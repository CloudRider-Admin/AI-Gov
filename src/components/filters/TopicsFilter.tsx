...
export function TopicsFilter({ categories }: TopicsFilterProps) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        ...
        <div className="relative flex-grow">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-terminal-dark border border-terminal-border rounded-lg font-mono text-sm text-terminal-text placeholder:text-terminal-muted focus:outline-none focus:border-terminal-green/50 transition-colors"
          />
        </div>
        ...
        <button
          ...
          className="sm:hidden flex items-center justify-center gap-2 px-4 py-2 border border-terminal-border rounded-lg text-terminal-muted hover:text-terminal-text hover:border-terminal-green/50 transition-colors"
        >
        ...
        <div className="hidden sm:block">
          <select
            ...
            className="px-3 py-2 bg-terminal-dark border border-terminal-border rounded-lg font-mono text-sm text-terminal-text focus:outline-none focus:border-terminal-green/50 transition-colors cursor-pointer"
          >
      ...
      {showFilters && (
        <div className="sm:hidden mb-6 p-4 bg-terminal-dark/50 border border-terminal-border rounded-lg">
          ...
          <select
            value={categoryFilter}
            onChange={(e) => setCategoryFilter(e.target.value)}
            className="w-full px-3 py-2 bg-terminal-dark border border-terminal-border rounded-lg font-mono text-sm text-terminal-text focus:outline-none focus:border-terminal-green/50"
          >
            <option value="all">All Categories</option>
            {categories.map((cat) => (
              <option key={cat.id} value={cat.id}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {category.topics.map((topic) => (
                    <Link
                      ...
                      className="group p-4 border border-terminal-border rounded-lg hover:border-terminal-green/50 hover:bg-terminal-green/5 transition-all"
                    >
