      ...
      <nav className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-1">
            <div className="w-10 h-8 border-2 border-terminal-green rounded flex items-center justify-center">
                ...
                {item.children ? (
                  ...
                  >
                    ...
                    {openDropdown === item.name && (
                      <div className="absolute top-full left-0 mt-1 w-48 bg-terminal-dark border border-terminal-border rounded-lg shadow-xl py-2">
            ...
            {isAuthenticated ? (
              <>
                {(session.user as { role?: string })?.role === 'ADMIN' && (
                  <Link
                    href="/admin"
                    className={`flex items-center gap-1 px-3 py-1.5 font-mono text-xs rounded-md transition-colors ${
                      pathname.startsWith('/admin')
                        ? 'bg-terminal-green/15 text-terminal-green'
                        : 'text-terminal-muted hover:text-terminal-green hover:bg-terminal-green/10'
                    }`}
                  >
                ...
                <span className="flex items-center gap-2 text-xs text-terminal-muted font-mono">
                  <User className="w-3 h-3" />
                  {session.user?.name ?? session.user?.email}
                </span>
                <button
                  onClick={() => signOut({ callbackUrl: '/' })}
                  className="flex items-center gap-1 px-4 py-2 font-mono text-sm text-terminal-muted hover:text-terminal-text transition-colors"
                >
              ...
              {isAuthenticated ? (
                <>
                  {(session.user as { role?: string })?.role === 'ADMIN' && (
                    <Link
                      href="/admin"
                      className="flex items-center justify-center gap-2 w-full px-4 py-2 font-mono text-sm text-terminal-green border border-terminal-green/30 rounded transition-colors hover:bg-terminal-green/10"
                      onClick={() => setMobileMenuOpen(false)}
                    >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 w-full px-4 py-2 font-mono text-sm text-terminal-muted hover:text-terminal-text border border-terminal-border rounded transition-colors"
                  >
                    <LogOut className="w-4 h-4" />
                    Sign Out
                  </button>
